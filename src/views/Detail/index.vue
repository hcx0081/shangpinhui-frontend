      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input v-model.number="count" autocomplete="off" class="itxt" @change="handleChange">
                <a class="plus" @click="count++">+</a>
                <a class="mins" @click="count>0?count--:count">-</a>
              </div>
              <div class="add">
                <a @click="addCart(skuInfo.id,count)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getDetail() {
      this.$store.dispatch('detail/detail', this.$route.params.skuId);
    },
    changeActive(list, spuSaleAttrValue) {
      list.map((el) => {
        el.isChecked = '0';
      });
    ...
    async addCart(id, count) {
      try {
        await this.$store.dispatch('shoppingCart/addOrUpdateShoppingCart', { skuId: id, skuNum: count });
